...
from wsapi4plone.core.tests.expected_results import (
    PORTAL_SKELETON, PORTAL_VALUES)
...
class BaseTestCase(PloneTestCase.PloneTestCase):
    ...
    def afterSetUp(self):
        ...
        from wsapi4plone.core.services import PloneRootService
        sm.registerAdapter(PloneRootService,
    ...
    def afterTearDown(self):
        ...
        from wsapi4plone.core.interfaces import IServiceContainer
        from wsapi4plone.core.services import PloneRootService
        sm.unregisterAdapter(factory=PloneRootService,
                             required=(IPloneSiteRoot,),
                             provided=IServiceContainer)
...
class BasePloneRootServiceTestCase(BaseTestCase):
    ...
    def afterSetUp(self):
        ...
        self.portal.manage_setLocalRoles('test_user_1_',
...
class BasePloneContentsExtensionTestCase(BasePloneRootServiceTestCase):
    ...
    def afterSetUp(self):
        ...
        from wsapi4plone.core.extensions.plone import IPloneContents
        sm.registerUtility(IPloneContents,
                           provided=IServiceExtension,
                           name='contents')
        # Register the extension itself
        from wsapi4plone.core.interfaces import IServiceContainer
        from wsapi4plone.core.extensions.plone import PloneContents
        sm.registerAdapter(PloneContents,
        ...
        # achieve its purpose.
        from wsapi4plone.core.extensions.plone import (
            IContentsQuery, PloneFolderContents)
        sm.registerAdapter(PloneFolderContents,
                           required=(IContainer,),
                           provided=IContentsQuery)
    ...
    def beforeTearDown(self):
        ...
        from wsapi4plone.core.interfaces import IServiceExtension
        from wsapi4plone.core.extensions.plone import IPloneContents
        sm.unregisterUtility(IPloneContents,
                             provided=IServiceExtension,
                             name='contents')
        # Register the extension itself
        from wsapi4plone.core.interfaces import IServiceContainer
        from wsapi4plone.core.extensions.plone import PloneContents
        sm.unregisterAdapter(factory=PloneContents,
                             required=(IServiceContainer, IContainer,),
                             provided=IPloneContents,
                             name='contents')
        # Register the adapter(s) and utilities used by the extension to
        ...
        from wsapi4plone.core.extensions.plone import (
            IContentsQuery, PloneFolderContents)
        sm.unregisterAdapter(factory=PloneFolderContents,
                             required=(IContainer,),
                             provided=IContentsQuery)
        # IFormatQueryResults is registered for IContentsQuery
        from wsapi4plone.core.utilities.interfaces import IFormatQueryResults
...
class TestPloneContentsExtensions(BasePloneContentsExtensionTestCase):
    ...
    def test_get_PloneFolderContents(self):
        ...
        ext = getMultiAdapter((self.serviced_site, self.serviced_site.context),
                              IExtension, name=u'contents')
        contents = ext.get()
        content_uri = u'/plone/image4'
        self.failUnless(content_uri in contents,
                        "Failed to get the contents of the folder.")
        self.logout()
